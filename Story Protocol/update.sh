...
init_cosmovisor() {
    ...
    if ! wget -p $HOME/story-v0.13.0 https://github.com/piplabs/story/releases/download/v0.13.0/story-linux-amd64 -O $HOME/story-v0.13.0/story; then
        echo "Failed to download the genesis binary. Exiting."
        exit 1
    fi

    # Initialize cosmovisor
    if ! cosmovisor init $HOME/story-v0.13.0/story; then
